...
--
-- Host: 127.0.0.1
-- Czas generowania: 03 Mar 2024, 11:34
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `pomieszczenie1`  AS SELECT `malowanie`.`id_sciany` AS `id_sciany`, `malowanie`.`id_farby` AS `id_farby`, `malowanie`.`liczba_puszek` AS `liczba_puszek` FROM `malowanie` WHERE `malowanie`.`id_pomieszczenia` = 11  ;
